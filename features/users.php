...
class User  {
	...
	public function __construct($decoded_data) {
			...
			//Hash + Salted password. Never store plain text password in database.

			$plain_text_password = $decoded_data["password"];

			$hashed_password = create_hash($plain_text_password);
	...
	static function user_login($username,$user_entered_password){
		...
		$STH = $DBH->prepare("SELECT user_id,password,secret_code FROM users WHERE user_name = ?");
		...
		$good_hash = $returned_row["password"];
	  

	  

		if(!validate_password($user_entered_password, $good_hash)){
	...
	//util helper function to take all plain text, non null passwords and encrypt them
	//once the function is executed, old passwords cannot be recovered
	static function encrypt_passwords(){
		...
		while ($returned_row = $STH->fetch(PDO::FETCH_ASSOC)) {
			...
			$plain_text_password = $returned_row["password"];
			$user_id = $returned_row["user_id"];

			if($plain_text_password != NULL AND !strstr($plain_text_password,"sha256")){

				$encrypted_password = create_hash($plain_text_password);


				//get all users
				$STH1 = $DBH->prepare("UPDATE users SET password = ? WHERE user_id = ?");

				$STH1->bindParam(1, $encrypted_password); 
				$STH1->bindParam(2, $user_id); 

				$STH1->execute(); 

				echo "password for user " . $returned_row["user_name"] . " has been changed: " . $plain_text_password . "-> " .  $encrypted_password . "<br>";
